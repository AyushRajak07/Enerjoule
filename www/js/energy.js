...
export async function initEnergy() {
  ...
  if (history.length) {
    // Stored history rows use `timestamp`; the live reading uses `lastUpdate`.
    const last = history[history.length - 1];
    currentReading = {
      voltage: Number.isFinite(last.voltage) ? last.voltage : settings.energy?.baseVoltage || 230,
      power: Number.isFinite(last.power) ? last.power : 0,
      energyConsumption: Number.isFinite(last.energyConsumption) ? last.energyConsumption : 0,
      lastUpdate: Date.now(),
    };
  } else {
    currentReading = {
...
export function accumulateEnergy(powerWatts, now = Date.now()) {
  ...
  const sinceLast = Number.isFinite(currentReading.lastUpdate) ? now - currentReading.lastUpdate : 0;
  const elapsedMs = Math.min(Math.max(0, sinceLast), 60000);
